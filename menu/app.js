...
// adding filter buttons dynamically to the HTML page
displayMenuButtons = () => {
  const categories = menu.reduce( (values, item)=> {
    if (!values.includes(item.category)) {  // select unique categories from the menu array
      values.push(item.category);
    }
    return values;
  }, ["all"]);
  const categoryBtns = categories.map(category => `<button class="filter-btn" type="button" data-id="${category}">${category}</button>`).join("");
  btnContainer.innerHTML = categoryBtns;
  // NOTE: if you add some content dynamically, you can only access it once it has been added to the html DOM.
...
/*
  Learnings:
  1. filter, map, join, reduce, include methods
  2. returning html display content dynamically
  3. dataset property(in js) and data-{property name} (in html)
  4. adding buttons dynamically to html
*/