...
const loremForm = document.querySelector(".lorum-form");
const loremText = document.querySelector(".lorem-text");
const amount = document.getElementById("amount"); // user input

loremForm.addEventListener("submit", (e) => {
  e.preventDefault(); // a forms' default behaviour is to submit it; therefore we use preventDefault() here.
  const amount = parseInt(this.amount.value); //use 'this' to take global value in case of assigning to the same variable name in local
  let randomNumber = Math.floor(Math.random() * text.length);

  if (isNaN(amount) || (amount < 0) || (amount > 9)) { //isNaN() -> is not a number
...
/* 
  Learnings:
  1. submit is default behaviour of forms.
  2. use of 'this' to access global variable.
  3. inputs are always in form of string, so use parseInt() method.
  4. isNaN() method to check if given variable is of number type.
  5. slice() method to take specific content from array.
*/